...
- **`app.js`** owns a single global `state` object (`{ years, vacations, profile }`), persisted to `localStorage` via `saveState()`/`loadState()`, and calls `calculate(state)` from `calc.js`. Rendering is a manual, non-reactive pipeline — `render()` calls `renderYears()`/`renderVacations()`/`renderResults()`/`renderProfile()` — re-triggered explicitly from input/click handlers (`onYearInput`, `onVacationInput`, `onYearsClick`, `onVacationsClick`).
- **`pdf-io.js`** handles PDF export/import on top of the vendored `pdf-lib`/`fontkit`. `exportPdf()` draws a report mirroring the Results view using `PdfReport`, a small hand-rolled layout helper around pdf-lib's low-level drawing API (manual page-break/positioning logic, not a templating layer), and embeds the full `state` as a hidden JSON file attachment inside the generated PDF. `drawFooters()` adds the per-page branding link/date footer as a separate pass over `pdfDoc.getPages()` *after* the results section is fully drawn, since the results section decides the page count via `PdfReport`'s dynamic `addPage()`/`ensureSpace()` calls. `importPdfFile()` reverses this (`extractEmbeddedJson`/`collectNameTreeLeaves`) to pull that attachment back out and restore `state`.
